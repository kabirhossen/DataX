	...
	async function onClickConnect(type) {
		if (type === 1) {
			...
			if (window?.tronWeb?.defaultAddress?.base58 != null) {
				...
				window.localStorage.setItem("type", "keplr")

			} else {
				setisKeplrConnected(false);
			}
		} else {
			window.localStorage.setItem("type", "non-Keplr")
			await fD();
			setisKeplrConnected(true);
		}
	}
						...
						{isKeplrConnected ? (
							...
							<>
								<button
									...
									className="bg-orange-500 text-white rounded-md shadow-md h-10 w-full mt-3 hover:bg-orange-600 transition-colors overflow:hidden flex content-center items-center justify-center cursor-pointer"
								>
									<span id="buttonText">Connect Keplr</span>
								</button>
								<button
									onClick={(e) => {
										onClickConnect(0);
								...
								>
									<span id="buttonText">Continue without Keplr</span>
								</button>
							</>
						)}
