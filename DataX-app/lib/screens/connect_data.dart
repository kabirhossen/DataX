...
import 'package:DataX/components/data_edit_item.dart';
...
class ConnectDataApp extends State<ConnectDataScreen> {
  ...
  Future<void> ConnectData() async {
    ...
    try {
        var url = Uri.parse(
        'https://DataX-tron-s4-api.onrender.com/api/POST/UpadateFhir');
  ...
  Future<void> GetData() async {
    final prefs = await SharedPreferences.getInstance();
    var userid = prefs.getString("userid");
   var url = Uri.parse('https://DataX-tron-s4-api.onrender.com/api/GET/getFhir?userid=${int.parse(userid.toString())}');
